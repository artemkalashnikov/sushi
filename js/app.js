...
function openMenu(block, button) {
  var wrapper = document.querySelector(block),
      headerButton = document.querySelector(button);

  headerButton.addEventListener('click', function () {
    wrapper.classList.toggle('wrapper_opened');
  });
}

function fixedCol(fixed_col, header) {
  var fixedCol = document.querySelector(fixed_col),
      fixedColMarginTop = 20,
      fixedColTop = document.querySelector(header).offsetHeight - fixedColMarginTop;
  if (fixedCol) {
    window.addEventListener('scroll', function () {
      if (window.innerWidth >= 666) {
        var scroll = window.pageYOffset;
        if (scroll > fixedColTop) {
          fixedCol.classList.add('_fixed');
          fixedCol.style.top = fixedColMarginTop + 'px';
        } else {
          fixedCol.classList.remove('_fixed');
          fixedCol.style.top = '';
        }
      } else {
        fixedCol.classList.remove('_fixed');
...
function preLoaderHide(preloaderWrapper) {
  var preLoader = document.querySelector(preloaderWrapper);
...
function zoomImg(allImages, popUp, popUpImg, popUpBackground, button, form) {
  var zoomItems = document.querySelectorAll(allImages),
      popUpWindow = document.querySelector(popUp),
      popUpBg = document.querySelector(popUpBackground),
      btn = document.querySelector(button),
      popUpForm = document.querySelector(form),
      img = document.querySelector(popUpImg);

  if (zoomItems) {
    for (var i = 0; i < zoomItems.length; i++) {

      zoomItems[i].addEventListener('click', function (e) {
        popUpWindow.style.zIndex = 5;
        img.setAttribute('src', e.target.getAttribute('src'));
        popUpForm.style.display = 'none';
        img.style.display = '';
        popUpWindow.classList.add('pop-up_opened');
      });
    }
    ...
    popUpBg.addEventListener('click', function () {
      popUpWindow.classList.remove('pop-up_opened');
      setTimeout(function () {
        popUpForm.style.display = 'none';
        img.style.display = 'none';
        popUpWindow.style.zIndex = '';
      }, 500);
    });
  }
}

function showPopUp(button, popUp, popUpForm, popUpBackground) {
...
function mobileCrutch(from, parent) {
  if (window.location.pathname == '/' || window.location.pathname == '/index.html') {} else {
    var blockFrom = document.querySelector(from),
        blockIn = document.querySelector(parent),
    ...
    if (window.innerWidth <= 666) {
      if (menu) {
        menu.style.display = 'none'; // Hide menu on other pages
      }
    }

    div.className = 'mobile-content';
    div.innerHTML = blockFrom.innerHTML;
    mobileHidden = div.querySelectorAll('[data-mobile]');
    for (var i = 0; i < mobileHidden.length; i++) {
      mobileHidden[i].style.display = 'none';
    }
    blockIn.appendChild(div);
  }
}

window.addEventListener('load', function () {
  openMenu('.wrapper', '.header__outer-menu');
  fixedCol('.left-column__content', '.header');
  fixedCol('.right-column__content', '.header');
  swipeMenu('.wrapper');
  zoomImg('.zoom-picture', '.pop-up', '.pop-up__img img', '.pop-up__window', '.btn_form', '.pop-up__form');
  mobileCrutch('.main', '.content');
}, false);
preLoaderHide('.preLoader');